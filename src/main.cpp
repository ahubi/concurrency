...
#define LOG(x)                                                        \
  do {                                                                \
    cout << hex << "[" << this_thread::get_id() << "] " << x << endl; \
  } while (0)
  ...
  inline void operator()() {
    if (mData) {
      ...
      while (1) {
        ...
        if (mData->mDataVector.size() >
            0) {  // and locked again after thread is leaving waiting
        ...
        this_thread::sleep_for(chrono::microseconds(
            100));  // sleep just to simulate run time of a thread
      }
    }
  };
...
void consumer(shared_ptr<thread_data> pSharedData) {
  if (pSharedData) {
    ...
    while (1) {
      ...
      if (pSharedData->mDataVector.size() >
          0) {  // and locked again after thread is leaving waiting
  ...
  if (pSharedData) {
    ...
    while (1) {
      unique_lock<mutex> lck(
          pSharedData->mMutex);  // lock to be able to write to data
      ...
      LOG("producer: " << pSharedData->mDataVector.size()
                       << " elements in queue");

      pSharedData->mCondVar.notify_all();  // notify all waiting threads
      lck.unlock();  // unlock the mutex to allow other threads to be able to
                     // access the data

      this_thread::sleep_for(chrono::milliseconds(
          1000));  // sleep just to simulate run time of a thread
    }
  }
}
...
void run_future_promise() {
  int numberCommands;
  CommandQueue cmdQ;
  thread cmdQThread(&CommandQueue::run, &cmdQ);
  while (1) {
    cout << "Please enter number of commands to be run:" << endl;
    cin >> numberCommands;
    for (size_t i = 0; i < numberCommands; i++) {
      string strCommand = "Command" + to_string(i);
      Command cmd = Command(strCommand);
      cmdQ.addCommand(cmd);
      cout << "Command " << i <<  " added" << endl;
    }
  }
  cmdQThread.join();
}
